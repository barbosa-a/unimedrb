...
$wpp = filter_input(INPUT_POST, 'wpp', FILTER_DEFAULT);

if ($wpp) {
    //Verificar se o email esta cadastrado no banco de dados
    $verifica_registro = "SELECT id_user, numeroCelular, usuarios_autenticacao_id FROM usuarios WHERE numeroCelular = '$wpp' LIMIT 1";
    $query_verifica_registro = mysqli_query($conn, $verifica_registro);

    if (($query_verifica_registro) and ($query_verifica_registro->num_rows > 0)) {

        $user = mysqli_fetch_assoc($query_verifica_registro);
            ...
            try {
                ...
                if (mysqli_affected_rows($conn) != 0) {
                    ...
                    $arr = array(
                        'tipo' => 'success',
                        'titulo' => 'Sucesso',
                        'msg' => 'Nova senha enviada para o número de cadastro',
                        'usuario' => $user['id_user'],
                        'wpp' => $user['numeroCelular'],
                        'senha' => $novaSenha,
                        'modulo' => 1
                    );
        ...
        $arr = array('tipo' => 'info', 'titulo' => 'Atenção', 'msg' => 'Número inválido');
        echo json_encode($arr);
    }
} else {
    $arr = array('tipo' => 'info', 'titulo' => 'Atenção', 'msg' => 'Digite um número válido');
    echo json_encode($arr);
}
