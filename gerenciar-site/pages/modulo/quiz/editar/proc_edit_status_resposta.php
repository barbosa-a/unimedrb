<?php
    ...
    if (!empty($dados['id'])) {
        ...
        $cad = "UPDATE quiz_pergunta_respostas SET situacao = '{$dados['status']}', usuario_id = '{$_SESSION['usuarioID']}', modified = NOW() WHERE id = '{$dados['id']}' ";
        $query_cad = mysqli_query($conn, $cad);
        if (mysqli_affected_rows($conn) > 0) {
            // Retorna uma resposta em JSON para o JavaScript
            $response = array('tipo' => 'success', 'titulo' => 'Sucesso', 'msg' => 'Registro alterado com sucesso!');
