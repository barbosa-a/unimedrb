	...
	if (isset($_FILES['arquivo']['name'])) {
		...
		//O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
			...
			try {
                ...
                $caminho = "pages/modulo/meu_perfil/avatar/$nome_final";
				...
				if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){
					...
					$up = "UPDATE usuarios SET foto = '$caminho', modificado_user = NOW() WHERE token = '{$usuario}' ";
					$query = mysqli_query($conn, $up);
					if (mysqli_affected_rows($conn) > 0) {
						$msg = array(
							'tipo' => 'success', 
							'titulo' => 'Sucesso', 
							'msg' => 'Foto cadastrado com sucesso'
						);
						echo json_encode($msg);	
					}else{
						$msg = array(
							'tipo' => 'error', 
							'titulo' => 'Erro de processamento', 
							'msg' => 'Erro ao salvar o arquivo, tente novamente <br>'.mysqli_error($conn)
						);
						echo json_encode($msg);	
					}				
				}else{
