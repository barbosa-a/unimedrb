...
if (isset($_FILES['capaPub']['name'])) {

    $arquivo     = $_FILES['capaPub']['name'];
    ...
    $size = getimagesize($_FILES['capaPub']['tmp_name']);

    //Verifica se houve algum erro com o upload. Sem sim, exibe a mensagem do erro
    if ($_FILES['capaPub']['error'] != 0) {
        $msg = array(
            'tipo' => 'error',
            'titulo' => 'Erro de processamento',
            'msg' => 'Não foi possivel fazer o upload <br> erro: ' . $_UP['erros'][$_FILES['capaPub']['error']]
        );
    ...
    $extensao = pathinfo($_FILES['capaPub']['name'], PATHINFO_EXTENSION);
    ...
    else if ($_UP['tamanho'] < $_FILES['capaPub']['size']) {
        $msg = array(
            'tipo' => 'error',
            'titulo' => 'Erro de processamento',
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        try {
            ...
            if (move_uploaded_file($_FILES['capaPub']['tmp_name'], $_UP['pasta'] . $nome_final)) {
