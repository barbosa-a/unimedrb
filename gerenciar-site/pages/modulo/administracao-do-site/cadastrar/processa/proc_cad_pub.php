<?php  
  ...
  if (isset($_FILES['arquivo']['name'])) {
    ...
    $_UP['pasta'] = '../../../../../dist/storage/publicacoes/';
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        try {
            ...
            $noticia = str_replace("'", "", $dado['noticia']);
            $resumo = str_replace("'", "", $dado['resumo']);

            // Pasta
            $pasta = "/dist/storage/publicacoes/$nome_final";
            ...
            if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){
                ...
                $cad = "INSERT INTO site_publicacoes 
                        (titulo, subtitulo, texto, descricao_meta, palavras_chave, capa_princial, slug, status, pagina, destacar, unidade_id, site_publicacoes_categoria_id, curtidas, visualizacoes, usuario_id, created) 
                            VALUES
                        ('{$dado['titulo']}', '{$dado['subtitulo']}', '{$noticia}', '{$resumo}', '{$dado['palavras_chave']}', '{$pasta}', '{$slug}', '{$dado['status']}', '{$dado['pagina']}', '{$dado['destacar']}', '{$dado['unidade']}', '{$dado['categoriaPub']}', 0, 0, '{$_SESSION['usuarioID']}', NOW())
                ";                
                ...
                $msg = array(
                    'tipo' => 'success', 
                    'titulo' => 'Sucesso', 
                    'msg' => 'Postagem registrada com sucesso',
                    'id' => $id
                );
                echo json_encode($msg);	
