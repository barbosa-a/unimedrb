<?php  
  ...
  if (isset($_FILES['arquivo']['name'])) {
    ...
    $_UP['pasta'] = '../../../../../dist/storage/depoimento/';
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        try {           
            ...
            $slug = slugGeral($dado['nome']);

            //Cria um nome baseado no UNIX TIMESTAMP atual
            $nome_final = $slug . time().'.'.$extensao;

            // Pasta
            $pasta = "/dist/storage/depoimento/$nome_final";

            //Verificar se é possivel mover o arquivo para a pasta escolhida
            if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){

                //Upload efetuado com sucesso, exibe a mensagem
                $cad = "INSERT INTO site_depoimento 
                        (nome, cargoFuncao, texto, foto, usuario_id, created) 
                            VALUES
                        ('{$dado['nome']}', '{$dado['cargoFuncao']}', '{$dado['texto']}', '{$pasta}', '{$_SESSION['usuarioID']}', NOW())
                ";                
