  ...
  if (isset($_FILES['arquivo']['name'])) {
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        try {
            ...
            $nome_final = slugImage($_FILES['arquivo']['name']) . '.' . $extensao;

            // Pasta
            $pasta = "/dist/storage/logos/$nome_final";

            // Nome
            $nome = $_FILES['arquivo']['name'];

            // Converter hora em segundos
            $slug = slugImage($_FILES['arquivo']['name']);
            ...
            if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){
                ...
                $cad = "INSERT INTO site_logos 
                        (logo, slug, arquivo, local, usuario_id, created) 
                            VALUES
                        ('{$nome}', '{$slug}', '{$pasta}', 'Cabeçalho', '{$_SESSION['usuarioID']}', NOW())
                ";                
