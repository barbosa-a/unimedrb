  ...
  if (isset($_FILES['arquivo']['name'])) {
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        try {
            ...
            $nome_final = slugImage($dado['nome']) . "-" . date('s') . "." . $extensao;

            // Pasta
            $pasta = "/dist/storage/ceo/$nome_final";

            //Verificar se é possivel mover o arquivo para a pasta escolhida
            if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){

                //Upload efetuado com sucesso, exibe a mensagem
                $cad = "INSERT INTO site_ceo 
                        (nome, descricao, biografia, data_inicio, data_fim, foto, usuario_id, created) 
                            VALUES
                        ('{$dado['nome']}', '{$dado['descricao']}', '{$dado['biografia']}', '{$dado['data_inicio']}', '{$dado['data_fim']}', '{$pasta}', '{$_SESSION['usuarioID']}', NOW())
                ";                
