<?php  
  ...
  if (!empty($_FILES['arquivo']['name'])) {
    ...
    elseif (($size[0] < '750' OR $size[1] < '111') AND ($size[0] > '750' OR $size[1] > '111')) {
        $msg = array(
            'tipo' => 'error', 
            'titulo' => 'Dimensões inválidas', 
            'msg' => 'As dimensões permitidas para o anúncio é: 750 x 111'
        );
            ...
            if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta']. $nome_final)){
                ...
                $cad = "INSERT INTO site_banners_ads 
                        (titulo, legenda, slug, arquivo, link, anuncio, usuario_id, created) 
                            VALUES
                        ('{$dado['titulo']}', '{$dado['legenda']}', '{$slug}', '{$pasta}', '{$dado['link']}', 'Anúncio 03', '{$_SESSION['usuarioID']}', NOW())
                ";                
    ...
    try {
        ...
        $cad = "INSERT INTO site_banners_ads 
                (titulo, legenda, slug, link, anuncio, usuario_id, created) 
                    VALUES
                ('{$dado['titulo']}', '{$dado['legenda']}', '{$slug}', '{$dado['link']}', 'Anúncio 03', '{$_SESSION['usuarioID']}', NOW())
        ";                
        $query = mysqli_query($conn, $cad);
